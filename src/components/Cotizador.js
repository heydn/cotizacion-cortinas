  ...
  const handleItemChange = (id, field, value) => {
    const updatedItems = items.map((item) => {
      if (item.id === id) {
        ...
        // Si se cambia el producto, restablecer el material
        if (field === "productoId") {
          item.materialId = ""; // Restablecer material cuando se cambia el producto
        }
          ...
          {items.map((item) => (
            ...
            >
              ...
              <select
                ...
                onChange={(e) =>
                  handleItemChange(item.id, "productoId", e.target.value)
                }
                className="border p-2 w-full mb-2"
              >
              ...
              {/* Selección de material basado en producto */}
              {item.productoId && (
                <select
                  value={item.materialId}
                  onChange={(e) =>
                    handleItemChange(item.id, "materialId", e.target.value)
                  }
                  className="border p-2 w-full mb-2"
                >
                  <option value="">Seleccionar Material</option>
                  {materiales[item.productoId]?.map((material) => (
                    <option
                      key={material.id}
                      value={material.id}
                    >
